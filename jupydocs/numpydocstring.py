...
class NumpyDocString:
    ...
    def __init__(self, function, h1="##", h2="###"):
        """
        Parameters
        ----------
        function : [type]
        ...
        self.section_keywords_mapping = {
            "parameters": (
                ["parameter", "parameters", "params", "param"],
    ...
    def render_md(self, return_str=False, function_name="__name__"):
        ...
        function_name : '__name__' | str | None
            If '__name__' will use the functions name. If None will not include
            any title. If any other string is provided will use the provided
            string as a custom name.
        ...
        """
        if function_name == "__name__":
            title_text = self.function_name
        elif function_name is None:
            title_text = ""
        else:
            title_text = function_name
        docstring_sections = [
            f"{self.h2[0:-1]} {title_text}\n\n",
            self.description_,
        ]
    ...
    def parse_description(self):
        ...
        for idx, txt in enumerate(doc):
            ...
            if (
                ...
                and re.match(self.numpy_section_regex, txt.strip()) is not None
            ):
                param_end = idx - 2
                break
    ...
    def parse_parameters(self, start, end):
        ...
        for idx, txt in enumerate(doc):
            ...
            if (
                num_leading_white_spaces[idx] >= num_leading_white_spaces[idx - 1]
                and idx > 0
                and " : " not in txt
            ):
                if doc[idx - 1].strip() == "":
    ...
    def parse_code_blocks(self, doc):
        ...
        for idx, txt in enumerate(doc):
            ...
            elif (txt == "" and inside_code_block == True) or (
                idx == len(doc) - 1 and inside_code_block == True
            ):
                inside_code_block = False
                txt = txt + "\n```\n"
            else:
    ...
    def find_section(self, doc, section_keywords):
        ...
        for idx, txt in enumerate(doc):
            if idx == 0:
                continue
            if (
                re.match(self.numpy_section_regex, txt.strip())
                and doc[idx - 1].strip().lower() in section_keywords
            ):
                section_start = idx - 1
                continue
            if (
    ...
    def render_class(
        ...
        if render_init:
            ...
            methods = []
        for x in dir(self.function):
            if x[0:2] != "__" and x[-2] != "__":
                methods.append(x)
        docstring = []
        if render_class_doc_string:
            docstring.append(
                NumpyDocString(self.function, header_level).render_md(True)
            )
        # render a list of all methods belonging to the class
        methods_list_str = f"{header_level} Methods"
        for method in methods:
            methods_list_str += (
                f"\n - [{self.function_name}.{method}](#{self.function_name}{method})"
            )
        docstring.append(methods_list_str)
        # render the doc string for each method
        for method in methods:
            doc = NumpyDocString(
                getattr(self.function, method),
                header_level + "#",
                function_name=f"{self.function_name}.{method}",
            )
...
# %%
# import pandas as pd
# print(render_class(NumpyDocString, render_class_doc_string=True, return_str=True))
