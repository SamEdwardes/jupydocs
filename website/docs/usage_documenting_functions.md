...
| sort             | bool, default False                                  | Sort non-concatenation axis if it is not already aligned when `join` is 'outer'. This has no effect when ``join='inner'``, which already preserves the order of the non-concatenation axis. <br></br> .. versionadded:: 0.23.0 .. versionchanged:: 1.0.0 <br></br> Changed to not sort by default. <br></br>              |
